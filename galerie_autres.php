...
//Delimiter le nombre des maisons a afficher par page
	$photoParPage = 12;
	$totalMaisonsReq = $bdd->prepare('SELECT * FROM galeries,maisons WHERE galeries.IdMaison = maisons.IdM and maisons.StatutMaison = :statutMaison ORDER BY IdIm DESC');
	$totalMaisonsReq->execute(array('statutMaison' => $statutMaison));
	$totalMaisons = $totalMaisonsReq->rowCount();
	$pagesTotales = ceil($totalMaisons / $photoParPage);
			...
			?>
			  ...
			  <ul class="thumbnails">
			  	<?php 
				  	...
				  	$req = $bdd->prepare('SELECT * FROM galeries,maisons WHERE galeries.IdMaison = maisons.IdM and maisons.StatutMaison = :statutMaison ORDER BY IdIm DESC LIMIT '.$depart.','.$photoParPage);
				  	$req->execute(array('statutMaison' => $statutMaison ));
				...
				<li class="span2">
				  <div class="thumbnail">
					<a  href="themes/images/galerie/<?php echo $resultat['Galerie']; ?>" download ><img src="themes/images/galerie/<?php echo $resultat['Galerie']; ?>" alt="Télécharger l'image" title = "Télécharger l'image" style="height: 120px; width: 100%;" alt=""/><span class="badge badge-inverse" style="position: absolute; display:block; top: -4px;right: -18px; height:25px; width:30px; border-radius: 50px; text-align: left;padding-top: 10px;padding-right: 19px;">N°<?php echo $resultat['Num']; ?></span></a>
					<div class="caption">
					  <p style="text-align: justify;"><?php echo utf8_encode($resultat['DescriptionPhoto']); ?></p>
